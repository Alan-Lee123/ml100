import torch
from torch.nn import Module, Conv2d, Sequential, ReLU
from torch.nn.modules.linear import Linear
from torch.nn.modules.pooling import AdaptiveAvgPool2d, MaxPool2d
from BatchNormalization.BatchNormalization import BatchNormalization2D
...
def make_conv_layer(in_channels, out_channels, kernel_size, padding):
    return Sequential(
        ...
        BatchNormalization2D(out_channels),
        ReLU(inplace=True)
    )
...
class Resnet_Basic_Block(Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.bn1 = BatchNormalization2D(out_channels)
        self.relu1 = ReLU(inplace=True)
        self.conv2 = conv3x3(out_channels, out_channels, stride=1)
        self.bn2 = BatchNormalization2D(out_channels)
        self.relu2 = ReLU(inplace=True)
        if in_channels != out_channels or stride != 1:
            self.downsample = Sequential(
                Conv2d(in_channels, out_channels, stride=stride, kernel_size=1, bias=False),
                BatchNormalization2D(out_channels)
            )
        else:
